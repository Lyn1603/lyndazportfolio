...
import springLogo from "@/assets/logos/spring-boot.svg";
...
import photoshopLogo from "@/assets/logos/adobe-photoshop.svg";
...
export const Route = createFileRoute("/")({
    head: () => ({
        meta: [
            ...
            {
                name: "description",
                content: "Personal portfolio: projects, skills and story of Lynda Benabdessadok, junior web developer from Paris."
            },
...
function Index() {
    return (
        <div
            className="starry-gradient min-h-screen font-body text-lavender-glow selection:bg-accent-pink selection:text-indigo-night">
            {/* Star Layer */}
            ...
            {/* Navigation */}
            <nav
                className="sticky top-0 z-50 flex items-center justify-between border-b border-border-dim px-6 py-4 backdrop-blur-md">
                ...
                {projects.map((p) => (
                    <section key={p.idx} className="grid grid-cols-1 items-start gap-12 lg:grid-cols-12">
                        <div className={`space-y-6 lg:sticky lg:top-32 lg:col-span-4 ${p.reverse ? "lg:order-2" : ""}`}>
                            ...
                            >
                                Visit Live <span
                                className="inline-block transition-transform group-hover:translate-x-1">→</span>
                            </a>
                        </div>
                                ...
                                <div key={category.title} className="space-y-4">
                                    ...
                                    <div className="flex flex-wrap gap-6">
                                        {category.items.map((item) => (
                                            ...
                                            >
                                                <div
                                                    className="flex h-14 w-14 items-center justify-center rounded-xl border border-border-dim bg-white p-2">
                                                    <img
                                                        src={item.logo}
                                                        alt={item.name}
                                                ...
                                                <span
                                                    className="font-mono text-[10px] text-center uppercase opacity-70">
                                                    {item.name}
                                                </span>
                                            </div>
                                        ))}
            ...
            <section className="mx-auto max-w-7xl px-6 py-24">
                <div className="border-t border-border-dim pt-12">
                    ...
                    <div className="flex items-center gap-4">
                        <a href="https://directory.opquast.com/fr/certificat/BZGDEW/" target="_blank" rel="noreferrer"
                        ...
                        <div className="flex-1">
                            <a href="https://directory.opquast.com/fr/certificat/BZGDEW/" target="_blank"
                               rel="noreferrer"
                               className="font-display text-2xl uppercase text-star-bright underline decoration-accent-pink underline-offset-4 hover:text-accent-pink">
